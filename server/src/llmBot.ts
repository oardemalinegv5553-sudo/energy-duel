...
### 特殊招式
- **欧**(Lv.7,0费)：偷目标本回合的气收益（2倍），是7级之后的常见开局招（运、欧、跺）之一，也是开局博取优势的重要一招
- **跺**(Lv.7,0费)：counter-kill任何对自己用欧的人，对其他人无效。是7级之后的常见开局招（运、欧、跺）之一
...
## 决策原则：**重要**
1. **剩者为王**：我们的目的是：出招杀死对手，且自己不死，所以除了进攻，要利用好防御招（防、超防、莲花、园丁、金牛、海王），防住对方可能的进攻
2. **稳中求进**：气少于别人时，虽然优先要防守，但是也可以出其不意蓄气，甚至发动小进攻。不要怕死，勇敢在大家0气时出欧。
3. **分清同类型出招优劣**：比如防防不住挂机，但是超防可以，所以绝境中考虑超防；龙盾能防住骇天，超防防不住骇天，而对手有可能出骇天或挂机时，我们就要双重考虑。
4. **注意使用特殊招式**：超防防挂机，欧偷气，跺反制欧，龙盾防骇天，毒盾防毒，莲花宝座霸体2回合，金牛漩涡顶、海王震天击碎园丁/金牛/海王，欧和跺是免费的
...
8. **注意特殊招式**：欧、跺、龙盾、毒盾、莲花宝座等有特殊规则，要合理利用。要求你每次开局（所有人0气）时保持相当的概率选择欧
9. **注意多人对战**：全体攻击（all类）会打到除自己外的所有活着的玩家；双目标攻击（dual类）可选择1-2个目标
10. **注意对手的策略**：对手可能会蓄气、破防、击碎、偷能量等，要根据他们的行为调整策略
11. **策略多变**：不要只想着最优解，多变策略和出其不意往往也能攻下一城
12. **0费招式比较**：开局：欧>跺>运，推理别人想做的再抗衡。你出运被欧偷了你就没气了，别他妈一直运。
...
### Lv.7
- 欧(cost0,special,single): 偷目标能量（若目标出运则偷2气，对方欧别人得的气也可被双倍夺取），被跺counter-kill，所以灵活欧是重要的。**欧和跺是这个游戏的核心玩法**
...
### Lv.9
- 牛(cost1,attack,ATK=30,single)
- 牛气冲天(cost3,attack,ATK=75,single)

### Lv.10
- 园丁(cost1,defense,DEF=75)
...
function buildUserMessage(
  ...
  // Strategic hints based on state
  const hints: string[] = [];
  if (bot.energy < 0.01 && others.every(o => o.energy < 0.01)) {
    hints.push('开局0气：欧(偷气)>跺(防欧)>运，出运可能被欧偷');
  }
  if (others.some(o => o.energy < 0.5)) {
    const cheapAtks = available.filter(m => m.atk > 0 && m.cost <= bot.energy);
    if (cheapAtks.length > 0) hints.push('对手低气→攻击可能直接杀');
  }
  if (others.some(o => o.energy >= 1)) {
    hints.push('对手有气→可能攻击你，考虑防御或对攻');
  }
  if (bot.energy >= 2) {
    const big = available.filter(m => m.atk >= 50 && m.cost <= bot.energy);
    if (big.length > 0) hints.push('高能量→可出大招破防');
  }

  lines.push('');
  if (hints.length > 0) lines.push('【策略提示】' + hints.join('；'));
